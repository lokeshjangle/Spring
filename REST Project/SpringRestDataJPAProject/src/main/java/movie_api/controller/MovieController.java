...
//	@RequestMapping("/movie-api/{movie_id}")
	...
	public void addNewMovie(@RequestBody Movie newMovie) {  //@RequestBody is use to collect and store data from form which filled by user in variable the data will be in JSON format
		//@ResponseBody is use to send the data to webpage which stores in database
		...
		if(foundMovie != null) {
			throw new MovieNotFoundException("Unable to find movie with this ID", newMovie.getMovieId());
		}
		else {
	...
	public void deleteMovie(@PathVariable("movieId") Integer movieId) {  //@PathVariable : is use to get user parameter from url and mapped with method parameter
		Movie foundMovie = movieServiceRef.getOneMovie(movieId);
		if(foundMovie == null) {
